...
WORKOUT_REP_COUNTER_APP_TITLE = "Hailo Workout Rep Counter App"


# -----------------------------------------------------------------------------------------------
# User Gstreamer Application
# -----------------------------------------------------------------------------------------------
class GStreamerWorkoutRepCounterApp(GStreamerApp):
    def __init__(self, app_callback, user_data, parser=None):
        ...
        parser.add_argument(
            ...
            default="squat",
            help="Type of exercise to count reps for (default: squat)",
        )

        # Handle --list-models flag before full initialization
        handle_list_models_flag(parser, POSE_ESTIMATION_PIPELINE)

        hailo_logger.info("Initializing GStreamer Workout Rep Counter App...")
        ...
        hailo_logger.debug(
            "Post-process SO: %s, Function: %s",
            self.post_process_so,
            self.post_process_function,
        )

        setproctitle.setproctitle(WORKOUT_REP_COUNTER_APP_TITLE)
        hailo_logger.debug("Process title set: %s", WORKOUT_REP_COUNTER_APP_TITLE)
...
def main():
    hailo_logger.info("Starting Workout Rep Counter App main()...")
    user_data = app_callback_class()
    app = GStreamerWorkoutRepCounterApp(dummy_callback, user_data)
    app.run()


if __name__ == "__main__":
    hailo_logger.info("Launching Workout Rep Counter App...")
    main()
