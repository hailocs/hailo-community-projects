...
| User says | Action |
|-----------|--------|
| `/hl-build-app` (no args) | Start **interactive discovery** (Phase 1) |
| `/hl-build-app <description>` | Parse intent from description, skip to **recommendation** (Phase 2) |
| `/hl-build-app list` | Show **catalog summary** organized by category |
| `/hl-build-app from <app-name>` | Start from a specific template, show **plan** (Phase 2.5) then **scaffold** (Phase 3) |
| `/hl-build-app standalone` | Constrain recommendations to standalone apps only |
| `/hl-build-app genai` | Constrain recommendations to GenAI apps only (Hailo-10H) |
| `/hl-build-app pipeline` | Constrain recommendations to pipeline apps only |
| `/hl-build-app clone-refs` | Clone external Hailo repos (TAPPAS, Model Zoo, HailoRT) into `reference_repos/` for code review |

### `list` Command

When the user runs `/hl-build-app list`:

1. Read `.hailo/knowledge/app_catalog.yaml`
...
> "Use `/hl-build-app from <name>` to start from any of these, or just describe what you want to build and I'll recommend the best fit."

### `from <app-name>` Command

When the user runs `/hl-build-app from <app-name>`:
...
When the user runs `/hl-build-app clone-refs`:
...
```
.hailo/knowledge/app_catalog.yaml
.hailo/knowledge/decision_tree.yaml
.hailo/knowledge/model_compatibility.yaml
```

Also consult `model_compatibility.yaml` to verify model/HEF availability for the user's target architecture before recommending any template.
...
### Step 1.5: Verify model compatibility

Before running any feasibility test, verify the recommended model is available for the user's architecture using `.claude/skills/hl-build-app/scripts/model_check.py`:

```bash
python .claude/skills/hl-build-app/scripts/model_check.py <model_name> <arch>
# Example: python .claude/skills/hl-build-app/scripts/model_check.py yolov8m hailo8l
```

This checks `knowledge/model_compatibility.yaml` for HEF availability, postprocess .so, and supported architectures.
...
The template docs in `.hailo/templates/` provide annotation and guidance, but the **real code comes from the source app**. For quick-start scaffolding, also consult the minimal examples in `.hailo/examples/`:
- `examples/minimal_detection/` — simplest pipeline detection app
- `examples/minimal_standalone/` — simplest standalone HailoRT app
- `examples/minimal_genai/` — simplest GenAI LLM chat app
- `examples/callback_patterns/` — isolated callback patterns (tracking, zones, line crossing, pose)
- `examples/display_patterns/` — display method examples (web UI, headless, hybrid overlay)

Always read the actual source files:
...
Use the template at `.hailo/templates/app_claude_md.md` as the basis. Replace all `{{placeholders}}` with actual values from the implementation plan.
...
- **Check code snippets first:** Read `.hailo/knowledge/code_snippets.yaml` for reusable patterns that match the use case:
...
For comprehensive validation, run the automated scripts from `.claude/skills/hl-build-app/scripts/`:

```bash
# Full validation (file checks, syntax, imports, common mistakes)
python .claude/skills/hl-build-app/scripts/validate_app.py community/apps/<type>_apps/<name>

# Smoke test (syntax, --help, import)
python .claude/skills/hl-build-app/scripts/test_scaffold.py community/apps/<type>_apps/<name>
```

Additionally, run these individual checks using the `Bash` tool:
...
>
> Just say the word and I'll run `/hl-profile` on your new app."

If the user agrees, suggest running:

```
/hl-profile community/apps/pipeline_apps/<new_name>/<new_name>.py
```
...
>
> Just say `/hl-contribute` and I'll walk you through it."
...
|------|-----------------|
| `.hailo/knowledge/app_catalog.yaml` | Phase 2 — choosing a template |
| `.hailo/knowledge/decision_tree.yaml` | Phase 2 — shortcut rules |
| `.hailo/knowledge/model_compatibility.yaml` | Phase 2 — model/HEF availability per architecture |
| `.hailo/knowledge/code_snippets.yaml` | Phase 4 — reusable callback patterns (pose angles, line crossing, zones, alerts, tracking state) |
| `.hailo/knowledge/troubleshooting.yaml` | Phase 5 — common errors and fixes |
| `.hailo/knowledge/best_practices.yaml` | Phase 3-5 — lessons learned from building apps |
| `.hailo/knowledge/pipeline_patterns.yaml` | Phase 3 — pipeline architecture patterns |
| `.hailo/templates/app_claude_md.md` | Phase 3.5 — per-app CLAUDE.md template |
| `.hailo/examples/` | Phase 3-4 — minimal runnable examples and callback patterns |
| `.claude/skills/hl-build-app/scripts/validate_app.py` | Phase 5 — automated app validation |
...
| `reference_repos/hailo_model_zoo/` | Phase 2-4 — model configs, retraining, compilation (clone with `/hl-build-app clone-refs`) |
| `reference_repos/hailort/` | Phase 4 — HailoRT API source and Python bindings (clone with `/hl-build-app clone-refs`) |

## Knowledge Base Reference

All knowledge files, examples, scripts, and templates in the app-builder skill:

### Knowledge Files (`knowledge/`)
| File | Purpose |
|------|---------|
| `app_catalog.yaml` | Structured catalog of all 26+ Hailo apps with models, features, and use cases |
| `decision_tree.yaml` | Decision tree and keyword shortcuts for app selection |
| `code_snippets.yaml` | Reusable callback patterns (pose, zones, line crossing, tracking, display) |
| `pipeline_patterns.yaml` | 8 GStreamer pipeline architecture patterns with code examples |
| `model_compatibility.yaml` | Model/HEF compatibility matrix — which models work on which architecture |
| `troubleshooting.yaml` | 20+ common errors with symptoms, causes, and fixes |
| `best_practices.yaml` | 15+ lessons learned from building Hailo apps |

### Templates (`templates/`)
| File | Purpose |
|------|---------|
| `pipeline_app.md` | Annotated template for pipeline GStreamer apps |
| `standalone_app.md` | Annotated template for standalone HailoRT apps |
| `genai_app.md` | Annotated template for GenAI LLM/VLM/STT apps |
| `app_claude_md.md` | Template for generating per-app CLAUDE.md files |

### Examples (`examples/`)
| Directory | Purpose |
|-----------|---------|
| `minimal_detection/` | Simplest pipeline detection app (callback + pipeline class) |
| `minimal_standalone/` | Simplest standalone HailoRT app |
| `minimal_genai/` | Simplest GenAI LLM chat app |
| `callback_patterns/tracking_callback.py` | Per-object tracking with state management |
| `callback_patterns/zone_counting_callback.py` | Polygon zone counting |
| `callback_patterns/line_crossing_callback.py` | Line crossing detection |
| `callback_patterns/pose_analysis_callback.py` | Pose keypoint extraction and joint angles |
| `display_patterns/web_ui_example.py` | Flask web control panel |
| `display_patterns/headless_example.py` | Headless/fakesink pipeline |
| `display_patterns/hybrid_overlay_example.py` | hailooverlay_community + user-frame hybrid |

### Scripts (`scripts/`)
| Script | Purpose |
|--------|---------|
| `validate_app.py` | Validate scaffolded app (files, syntax, imports, common mistakes) |
| `test_scaffold.py` | Smoke test scaffolded app (syntax, --help, import) |
| `model_check.py` | Verify model HEF availability for target architecture |
| `generate_readme.py` | Auto-generate README from app source code |
...
For detailed error diagnosis, consult `.hailo/knowledge/troubleshooting.yaml` which catalogs 20+ common errors with exact symptoms, causes, and step-by-step fixes organized by phase and app type.

| Error | Recovery |
|-------|----------|
| User's app name conflicts with existing app | Warn and suggest alternative name or confirm overwrite |
| Template app not found in catalog | Show available apps with `/hl-build-app list` |
