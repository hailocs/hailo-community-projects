...
- Confidence must be [0,1], theta is radians (-pi to pi), so use `std::to_string(theta)` in label
...
- **Bug**: `expand = |cos theta| + |sin theta|` changed with rotation → unstable crop size
- **Fix**: Fixed `expand = sqrt(2) = 1.41421356` in all three files
...
- All three C++ files (palm_croppers, hand_affine_warp, hand_landmark_postprocess) must
  use the SAME fixed expand factor (sqrt(2))
